...
#ifndef LALIB_PMATRIX_H
#define LALIB_PMATRIX_H
...
namespace bzx{
    ...
    */
    Matrix& diagonal(const size_t &_size,std::initializer_list<double> src){
        Matrix *res =  new Matrix(_size,_size);
        int i = 0;
        for(auto el:src){
            res->M[i][i] = el;
            ++i;
    ...
    */
    ...
    double _Determinant(const Matrix &subMat){
        ...
        Matrix *new_subMat = new Matrix(R-1,C-1);
        for(int j = 0; j < C; ++j){
            for(int r = 0; r < R;++r){
                for(int c = 0; c < C; ++c) {
                    if(r!=0 && c!=j){
    ...
    Matrix& adjoint_matrix(const Matrix &Mat){
        ...
        Matrix *sub_mat = new Matrix(R-1,C-1);
    ...
    std::tuple<Matrix&,Matrix&,Matrix&> SVD(const Matrix&Mat){
        ...
        Matrix* ATA = new Matrix();
        Matrix* AAT = new Matrix();
        Matrix* tmp = new Matrix();
        Matrix* U = new Matrix(), * Sigma = new Matrix(), * V = new Matrix();
        ...
        *Sigma = &SQRT(Sigma);
        ...
        return std::make_tuple(std::ref(*U),std::ref(*Sigma),std::ref(*V));
    }
    ...
    std::tuple<Matrix&,Matrix&> Jacobi(const Matrix&Mat){
        ...
        Matrix *EigenVector = &eye(R,C);
        Matrix *U = &eye(R, C);
        Matrix *UT = new Matrix;
        Matrix *EigenValue = new Matrix(R,1);
        Matrix copy_Mat = Mat;
        size_t max_iter = 1e4;
        size_t iter = 0;
        ...
        delete U, UT;
        return std::make_tuple(std::ref(*EigenValue),std::ref(*EigenVector));
    }
    ...
    std::tuple<double ,Matrix&> Power_method(const Matrix &Mat, const double &min_delta = 1e-5,const size_t &max_iter = 1e3){
        ...
        delete Y;
        Y = NULL;
        return std::make_tuple(M,std::ref(*X));
    }

    Matrix& SQRT(const Matrix& Mat) {
        Matrix *dst = new Matrix(Mat);
        size_t R, C, j, i;
